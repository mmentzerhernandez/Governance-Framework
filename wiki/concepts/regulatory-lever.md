---
title: The Regulatory Lever (Data Authority Guidance Note)
type: concept
tags: [lever, anchor, data-protection, package-component]
created: 2026-06-01
updated: 2026-06-01
sources: [ai-governance-global-majority-report]
---

# The Regulatory Lever

> **Package component #3.** Publish a Data Protection Authority Guidance Note (adapted from Brazil's
> ANPD) clarifying that existing data-privacy laws protect citizens against unchecked automated
> profiling and decisions.

## What it is
A guidance note issued by the national data protection authority that interprets the country's
**existing** data-protection statute to cover automated decision-making — establishing the right to
human review and to an explicit reason for automated profiling decisions. The model is
[[brazil-lgpd-anpd]] (LGPD Article 20 + ANPD Resolution 16/2024).

## Why no new law is needed
It uses the DPA's **administrative interpretive authority** over a law already on the books — most
African and Latin American countries had a data-protection law by 2024. No legislation; the note
clarifies how existing rights apply to algorithms. See [[zero-new-legislation-pathway]].

## Transferable mechanism (steps)
1. Confirm the existing data-protection law contains accuracy/transparency/purpose-limitation rights.
2. Draft the guidance note applying those rights to automated decisions and profiling.
3. Mandate the **right to human review** and an **explanation on request**.
4. Issue a DPIA template (modelled on ANPD Resolution 16) for high-risk government AI.
5. Optionally request technical cooperation from ANPD/SERPRO via South-South channels.

## Trigger / timing
**Anchor phase, Months 6–18.** Complements the [[redress-focal-point]] — the human-review right
needs an intake channel to be exercised.

## Cross-references
- [[brazil-lgpd-anpd]] · [[redress-focal-point]] · [[procurement-lever]]
- [[ca2-minimum-viable-governance]]
- [[overview]]
